...
TextQuery::TextQuery(ifstream &infile) : text(new vector<string>()) {
    ...
    string s1("\"Daddy, shush, there is no such thing.\"");
    string s(s1.length(), char());
    transform(s1.begin(), s1.end(), s.begin(), [](char ch) -> char { if ((ch < 65 || (ch > 90 && ch < 97) || ch > 122) && ch != ' ') return char(); return ch;});
    cout << s << endl;
    cout << s.length() << endl;
    istringstream iss(s);
    string s2;
    while (iss >> s2) {
        if (s2 == string(1, char())+"Daddy" + string(1, char())) cout << s2 << endl;
    }

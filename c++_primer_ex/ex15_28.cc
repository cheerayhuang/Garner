...
int main() {
    ...
    vector<Quote> v_quote = {Quote("a", 10.0), Quote("b", 14.5)};
    auto sum = 0.0;
    for (auto i : v_quote) {
        sum += i.NetPrice(10);
    }
    ...
    vector<shared_ptr<Quote>> v_quoteptr = {shared_ptr<Quote>(new Quote("a", 10.0)), shared_ptr<Quote>(new Quote("b", 11.0))};

    vector<shared_ptr<Quote>> v_quoteptr2 = {make_shared<Quote>("a", 10.0), make_shared<Quote>("b", 14.5)};

    sum = 0;
    for (auto i : v_quoteptr2) {
        sum += i->NetPrice(10);
    }
    cout << sum << endl;
